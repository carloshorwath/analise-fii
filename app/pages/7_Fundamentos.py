...
from app.components.data_loader import load_ticker_info, load_tickers_ativos
...
session = get_session()
info_row = session.execute(
    select(Ticker).where(Ticker.ticker == ticker)
).scalar_one_or_none()
if info_row is None:
    st.warning("Ticker nao encontrado no banco.")
    session.close()
    st.stop()
...
payout_df, consec = get_payout_historico(ticker, cnpj, 24, session)

if not payout_df.empty:
    if consec >= 3:
        st.error(f"ALERTA: distribuindo mais que gera por {consec} meses consecutivos")
...
dy_data = get_dy_medias(ticker, cnpj, session)
...
pvp_data = get_pvp_medias(ticker, cnpj, session)
...
if serie_pvp is not None and len(serie_pvp) > 0:
    ...
    fig_pvp.add_trace(go.Scatter(
        x=serie_pvp.index, y=serie_pvp.values,
...
pl_df = get_pl_cotas_historico(ticker, cnpj, 36, session)
