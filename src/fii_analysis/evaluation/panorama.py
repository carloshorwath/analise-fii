...
from src.fii_analysis.data.database import Dividendo, PrecoDiario, Ticker
...
def render_panorama(df=None):
    ...
    table.add_column("Rent. 12m", justify="right")
    table.add_column("Rent. 24m", justify="right")
    ...
    with get_session_ctx() as session:
        for _, row in df.iterrows():
            ...
            table.add_row(
                ...
                _fmt(row.get("rent_12m"), ".2f", pct=True),
                _fmt(row.get("rent_24m"), ".2f", pct=True),
                tipo,
                cvm_flag,
            )
        ...
        coletado_em_rows = {}
        for ticker_item in (df["ticker"].tolist() if df is not None else tickers_ativos(session)):
            ts = session.execute(
